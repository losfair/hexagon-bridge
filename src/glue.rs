...
#[no_mangle]
pub unsafe extern "C" fn hexagon_glue_destroy_cstring(s: *mut c_char) {
    CString::from_raw(s);
}

#[no_mangle]
pub unsafe extern "C" fn hexagon_glue_alloc(len: usize) -> *mut u8 {
...
#[no_mangle]
pub unsafe extern "C" fn hexagon_glue_free(mem: *mut u8) {
    let ptr_size = ::std::mem::size_of::<usize>();
...
fn test_alloc_free() {
    for i in 0..10000000 {
        unsafe {
            let m = hexagon_glue_alloc(i % 1024 + 1);
            *m = 42;
            hexagon_glue_free(m);
        }
    }
